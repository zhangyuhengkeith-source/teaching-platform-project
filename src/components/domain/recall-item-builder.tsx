  ...
  return (
    <div className="space-y-4 rounded-2xl border border-border bg-slate-50 p-5">
      <div className="flex items-start justify-between gap-4">
        <h3 className="text-base font-semibold">Term Recall Item {index + 1}</h3>
      ...
      <div className="space-y-3">
        ...
        {acceptedAnswers.fields.map((field, answerIndex) => (
          <div className="grid gap-3 md:grid-cols-[1fr_auto]" key={field.id}>
            <Input placeholder="Accepted answer" {...register(`items.${index}.answer_key_json.acceptedAnswers.${answerIndex}` as const)} />

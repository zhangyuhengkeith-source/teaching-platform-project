...
import { updateTaskSubmissionDraft } from "@/lib/mutations/electives";
...
type SubmissionActionResult =
  | { ok: true; data: TaskSubmissionSummary }
  | { ok: false; error: string };

export async function saveTaskSubmissionDraftAction(input: unknown): Promise<SubmissionActionResult> {
  try {
    ...
    const saved = await updateTaskSubmissionDraft(profile, { task, group }, parsed);
    ...
    console.error("saveTaskSubmissionDraftAction failed", error);
    return {
      ok: false,
      error: error instanceof Error ? error.message : "Failed to save submission draft.",
    };
  }
}
